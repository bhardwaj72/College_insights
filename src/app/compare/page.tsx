...
export default function ComparePage() {
  ...
  const filteredColleges = collegesData.filter((college: any) =>
    (college.name || college['Institute Name']).toLowerCase().includes(searchQuery.toLowerCase()) ||
    college.City.toLowerCase().includes(searchQuery.toLowerCase())
  ).slice(0, 5)

  const addCollege = (college: any) => {
    if (selectedColleges.length < 3 && !selectedColleges.find(c => c["Sr.No"] === college["Sr.No"])) {
  ...
  const removeCollege = (id: number) => {
    setSelectedColleges(selectedColleges.filter(c => c["Sr.No"] !== id))
  }
              ...
              <div className="mt-4 space-y-2">
                {filteredColleges.map((college: any) => (
                  <button
                    ...
                    disabled={selectedColleges.find(c => c["Sr.No"] === college["Sr.No"])}
                  ...
                  >
                    <div className="font-semibold text-[#2F2F2F]">{college.name || college['Institute Name']}</div>
        ...
        {selectedColleges.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {selectedColleges.map((college: any) => (
              <Card key={college["Sr.No"]} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-white/60 shadow-[0_6px_20px_rgba(0,0,0,0.08)] relative">
                <button
                  onClick={() => removeCollege(college["Sr.No"])}
                ...
                <CardContent className="p-6 space-y-4">
                  <div>
                    <h3 className="font-bold text-lg text-[#2F2F2F] mb-1">{college.name || college['Institute Name']}</h3>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center p-3 bg-gradient-to-r from-[#C9A7EB]/10 to-[#FFD6E0]/10 rounded-xl">
                      ...
                      <span className="font-bold text-[#8A2BE2]">#{college.nirfRank || college["Synthetic NIRF Rank"] || '-'}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-gradient-to-r from-[#00B4D8]/10 to-[#C9A7EB]/10 rounded-xl">
                      ...
                      <span className="font-bold text-[#00B4D8]">₹{college.avgPackage || Number(college.Avg_Package_LPA) || 0}L</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-gradient-to-r from-[#FFD6E0]/20 to-[#C9A7EB]/10 rounded-xl">
                      ...
                      <span className="font-bold text-[#8A2BE2]">₹{(((college.fees || (college.Fees_Lakh * 100000) || 0))/100000).toFixed(1)}L</span>
                    </div>
                    <div className="flex justify-between items-center p-3 bg-white/60 rounded-xl">
                      <span className="text-sm font-medium">Type</span>
                      <span className="font-bold text-[#2F2F2F]">{college.type || college.Ownership}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    <div className="text-center p-2 bg-gradient-to-br from-[#C9A7EB]/10 to-[#FFD6E0]/10 rounded-lg">
                      ...
                      <div className="font-bold text-[#8A2BE2]">{college.ratings?.placement || college.Placement_Score || 0}/5</div>
                    </div>
                    <div className="text-center p-2 bg-gradient-to-br from-[#00B4D8]/10 to-[#C9A7EB]/10 rounded-lg">
                      ...
                      <div className="font-bold text-[#00B4D8]">{college.ratings?.faculty || college.Faculty_Score || 0}/5</div>
                    </div>
                    <div className="text-center p-2 bg-gradient-to-br from-[#FFD6E0]/20 to-[#C9A7EB]/10 rounded-lg">
                      ...
                      <div className="font-bold text-[#C9A7EB]">{college.ratings?.campus || college.Campus_Score || 0}/5</div>
                    </div>
                    <div className="text-center p-2 bg-white/60 rounded-lg">
                      <div className="text-xs text-[#2F2F2F]/60 mb-1">ROI</div>
                      <div className="font-bold text-[#FFD6E0]">{college.ratings?.roi || college.ROI_Score || 0}/5</div>
                    </div>
                  </div>
